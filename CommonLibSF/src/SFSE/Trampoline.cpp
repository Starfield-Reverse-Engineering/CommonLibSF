...
namespace SFSE
{
	...
	{
		constexpr auto gigabyte = static_cast<std::size_t>(1) << 30;
		constexpr auto minRange = gigabyte * 2;
		constexpr auto maxAddr = std::numeric_limits<std::uintptr_t>::max();

		WinAPI::SYSTEM_INFO si;
		WinAPI::GetSystemInfo(&si);
		const auto granularity = si.allocationGranularity;

		auto       min = a_address >= minRange ? detail::roundup(a_address - minRange, granularity) : 0;
		const auto max = a_address < (maxAddr - minRange) ? detail::rounddown(a_address + minRange, granularity) : maxAddr;
